...
from matter_chip_tool_adapter.decoder import ChipLog
from runner import *
from tests_logger import TestColoredLogPrinter
...
def send_raw_command(command: str, server_path: str, server_arguments: str):
    ...
    log_printer.print(ChipLog.decode_logs(json_payload.get('logs')))
...
def chiptool_runner_options(f):
    f = click.option("--server_path", type=click.Path(exists=True), default=None,
                     required=False, help='Path to an websocket server to run at launch.')(f)
    f = click.option("--server_name", type=str, default='chip-tool',
                     required=False, help='Name of a websocket server to run at launch.')(f)
    f = click.option("--server_arguments", type=str, default='interactive server', required=False,
