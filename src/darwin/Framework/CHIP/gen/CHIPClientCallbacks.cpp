...
#define CHECK_MESSAGE_LENGTH(value)                                                                                                \
    ...
    if (messageLen < value)                                                                                                        \
    {                                                                                                                              \
        ChipLogError(Zcl, "Unexpected response length: %d < %d", messageLen, value);                                               \
        if (onFailureCallback != nullptr)                                                                                          \
        {                                                                                                                          \
            Callback::Callback<DefaultFailureCallback> * cb =                                                                      \
                Callback::Callback<DefaultFailureCallback>::FromCancelable(onFailureCallback);                                     \
...
bool emberAfReadAttributesResponseCallback(ClusterId clusterId, uint8_t * message, uint16_t messageLen)
{
    ...
    while (messageLen)
    {
        ...
        if (status == EMBER_ZCL_STATUS_SUCCESS)
        {
            ...
            case 0xF6: // List of endpoint ids
            {
                CHECK_MESSAGE_LENGTH(1);
                uint16_t count = chip::Encoding::Read8(message) / sizeof(EndpointId);
                ChipLogProgress(Zcl, "  EndpointIds: %lu", count);

                // TODO
                //
                // This is just here for making it easy to see the content of the attribute while developing.
                //
                while (messageLen)
                {
                    CHECK_MESSAGE_LENGTH(1);
                    EndpointId id = chip::Encoding::Read8(message);
                    ChipLogProgress(Zcl, "    EndpointId: 0x%02x", id);
                }

                Callback::Callback<EndpointIdsAttributeCallback> * cb =
                    Callback::Callback<EndpointIdsAttributeCallback>::FromCancelable(onSuccessCallback);
                cb->mCall(cb->mContext, count, message);
                break;
            }

            case 0xF7: // List of cluster ids
            {
                CHECK_MESSAGE_LENGTH(1);
                uint16_t count = chip::Encoding::Read8(message) / sizeof(ClusterId);
                ChipLogProgress(Zcl, "  ClusterIds: %lu", count);
                ...
                {
                    CHECK_MESSAGE_LENGTH(2);
                    ClusterId id = chip::Encoding::LittleEndian::Read16(message);
                    ChipLogProgress(Zcl, "    ClusterId: 0x%04x", id);
                }

                Callback::Callback<ClusterIdsAttributeCallback> * cb =
                    Callback::Callback<ClusterIdsAttributeCallback>::FromCancelable(onSuccessCallback);
                cb->mCall(cb->mContext, count, reinterpret_cast<ClusterId *>(message));
                break;
            }
