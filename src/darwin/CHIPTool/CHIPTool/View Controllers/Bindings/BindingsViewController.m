...
- (IBAction)bind:(id)sender
{
    ...
    [self.cluster bind:nodeId groupId:groupId endpointId:endpointId clusterId:clusterId completionHandler:^(NSError * _Nullable error, NSDictionary * _Nullable values) {
        NSString *resultString = (error == nil) ? @"Bind command: success!" :[NSString stringWithFormat:@"An error occured: 0x%02lx", error.code];
...
- (IBAction)unbind:(id)sender
{
    ...
    [self.cluster unbind:nodeId groupId:groupId endpointId:endpointId clusterId:clusterId completionHandler:^(NSError * _Nullable error, NSDictionary * _Nullable values) {
        NSString *resultString = (error == nil) ? @"Unbind command: success!" :[NSString stringWithFormat:@"An error occured: 0x%02lx", error.code];
        NSLog(resultString, nil);
    }];
}
