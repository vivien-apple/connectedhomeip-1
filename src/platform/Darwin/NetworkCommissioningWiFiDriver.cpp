...
// NOTE: DarwinWiFiDriver uses network config with empty ssid (ssidLen = 0) for empty network config.

// NOTE: For now, the DarwinWiFiDriver only supports one network, this can be fixed by using the wpa_supplicant API directly (then
...
CHIP_ERROR DarwinWiFiDriver::Shutdown()
{
    /*
...
CHIP_ERROR DarwinWiFiDriver::CommitConfiguration()
{
    /*
       ...
       mStagingNetwork.ssidLen)); ReturnErrorOnFailure(PersistedStorage::KeyValueStoreMgr().Put(kWiFiCredentialsKeyName,
       mStagingNetwork.credentials, mStagingNetwork.credentialsLen)); ReturnErrorOnFailure(ConnectivityMgrImpl().CommitConfig());
...
bool DarwinWiFiDriver::NetworkMatch(const WiFiNetwork & network, ByteSpan networkId)
{
    return networkId.size() == network.ssidLen && memcmp(networkId.data(), network.ssid, network.ssidLen) == 0;
}

Status DarwinWiFiDriver::AddOrUpdateNetwork(ByteSpan ssid, ByteSpan credentials, MutableCharSpan & outDebugText,
...
Status DarwinWiFiDriver::RemoveNetwork(ByteSpan networkId, MutableCharSpan & outDebugText, uint8_t & outNetworkIndex)
{
    /*
        outDebugText.reduce_size(0);
        outNetworkIndex = 0;
...
Status DarwinWiFiDriver::ReorderNetwork(ByteSpan networkId, uint8_t index, MutableCharSpan & outDebugText)
{
    outDebugText.reduce_size(0);
    VerifyOrReturnError(NetworkMatch(mStagingNetwork, networkId), Status::kNetworkIDNotFound);
...
void DarwinWiFiDriver::ConnectNetwork(ByteSpan networkId, ConnectCallback * callback)
{
    /*
        ...
        ChipLogProgress(NetworkProvisioning, "DarwinNetworkCommissioningDelegate: SSID: %s", networkId.data());
...
void DarwinWiFiDriver::ScanNetworks(ByteSpan ssid, WiFiDriver::ScanCallback * callback)
{
    /*
...
size_t DarwinWiFiDriver::WiFiNetworkIterator::Count()
{
    /*
...
bool DarwinWiFiDriver::WiFiNetworkIterator::Next(Network & item)
{
    /*
       ...
       && memcmp(configuredNetwork.networkID, item.networkID, item.networkIDLen) == 0)
        ...
        return true;
    */
    IgnoreUnusedVariable(driver);
    IgnoreUnusedVariable(exhausted);
    return false;
}
