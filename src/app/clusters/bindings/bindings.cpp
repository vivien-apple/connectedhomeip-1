...
EmberStatus prepareBinding(EmberBindingTableEntry & binding, NodeId nodeId, GroupId groupId, EndpointId endpointId,
                           ClusterId clusterId)
{
    if ((groupId && (nodeId != 0 || endpointId != 0)) || (nodeId && endpointId == 0))
    {
...
EmberStatus getBindingIndex(EmberBindingTableEntry & newEntry, uint8_t & bindingIndex)
{
    ...
    for (uint8_t i = 0; i < EMBER_BINDING_TABLE_SIZE; i++)
    {
        ...
        if (currentEntry.type == newEntry.type && currentEntry.clusterId == newEntry.clusterId &&
            currentEntry.local == newEntry.local)
        {
            if (currentEntry.type == EMBER_MULTICAST_BINDING && currentEntry.groupId != newEntry.groupId)
            {
                continue;
            }

            if (currentEntry.type == EMBER_UNICAST_BINDING &&
                (currentEntry.nodeId != newEntry.nodeId || currentEntry.remote != newEntry.remote))
            {
                continue;
            }
