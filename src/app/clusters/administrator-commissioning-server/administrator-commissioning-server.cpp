...
exit:
    if (status.HasValue())
    {
        ChipLogError(Zcl, "Failed to open commissioning window. Cluster status 0x%" PRIx8, status.Value());
        commandObj->AddClusterSpecificFailure(commandPath, static_cast<chip::ClusterStatus>(status.Value()));
    }
    else
    {
    ...
    const Commands::RevokeCommissioning::DecodableType & commandData)
{
    ...
    if (!Server::GetInstance().GetCommissioningWindowManager().IsCommissioningWindowOpen())
    {
        ...
        commandObj->AddClusterSpecificFailure(commandPath, static_cast<chip::ClusterStatus>(StatusCode::kWindowNotOpen));
    }
    else
    {
