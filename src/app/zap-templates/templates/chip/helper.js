 ...
 * @param {*} options
 */
function chip_server_has_complex_writable_attributes(options)
{
  const { clusterName } = checkIsInsideClusterBlock(this, 'chip_server_has_complex_writable_attributes');
  const attributes      = ensureClusters(this).getServerAttributes(clusterName);

  const filter = attribute => (attribute.isList || attribute.isStruct) && attribute.isWritable;
  return attributes.then(items => items.find(filter));
}
...
exports.chip_server_has_complex_writable_attributes          = chip_server_has_complex_writable_attributes;
