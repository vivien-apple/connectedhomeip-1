...
#define READ_ATTRIBUTES(name, mfgCode, cluster_id)                                                                                 \
    ...
    uint16_t result =                                                                                                              \
        encodeReadAttributesCommand(buffer, buf_length, mfgCode, destination_endpoint, cluster_id, attr_ids, attr_id_count);       \
    if (result == 0)                                                                                                               \
    {                                                                                                                              \
...
#define WRITE_ATTRIBUTE(name, mfgCode, cluster_id, value)                                                                          \
    ...
    if (_encodeGlobalCommand(buf, mfgCode, destination_endpoint, cluster_id, 0x02))                                                \
    {                                                                                                                              \
        buf.Put16(attr_id);                                                                                                        \
        buf.Put(attr_type);                                                                                                        \
        switch (attr_type)                                                                                                         \
...
#define REPORT_ATTRIBUTE(name, mfgCode, cluster_id, isAnalog, value)                                                               \
    BufBound buf = BufBound(buffer, buf_length);                                                                                   \
    if (_encodeGlobalCommand(buf, mfgCode, destination_endpoint, cluster_id, 0x06))                                                \
    {                                                                                                                              \
...
#define DISCOVER_ATTRIBUTES(name, mfgCode, cluster_id)                                                                             \
    BufBound buf = BufBound(buffer, buf_length);                                                                                   \
    if (_encodeGlobalCommand(buf, mfgCode, destination_endpoint, cluster_id, 0x0c))                                                \
    {                                                                                                                              \
...
#define COMMAND_HEADER(name, mfgCode, cluster_id, command_id)                                                                      \
    BufBound buf    = BufBound(buffer, buf_length);                                                                                \
    uint16_t result = _encodeClusterSpecificCommand(buf, mfgCode, destination_endpoint, cluster_id, command_id);                   \
    if (result == 0)                                                                                                               \
    {                                                                                                                              \
        ChipLogError(Zcl, "Error encoding %s command", name);                                                                      \
        return 0;                                                                                                                  \
...
uint16_t _encodeCommand(BufBound & buf, uint16_t mfgCode, EndpointId destination_endpoint, ClusterId cluster_id, CommandId command,
...
uint16_t _encodeClusterSpecificCommand(BufBound & buf, uint16_t mfgCode, EndpointId destination_endpoint, ClusterId cluster_id,
                                       CommandId command)
{
    ...
    if (mfgCode != EMBER_AF_NULL_MANUFACTURER_CODE)
    {
        frameControl = frameControl | (1u << 2);
    }

    return _encodeCommand(buf, mfgCode, destination_endpoint, cluster_id, command, frameControl);
}

uint16_t _encodeGlobalCommand(BufBound & buf, uint16_t mfgCode, EndpointId destination_endpoint, ClusterId cluster_id,
                              CommandId command)
{
...
uint16_t encodeReadAttributesCommand(uint8_t * buffer, uint16_t buf_length, uint16_t mfgCode, EndpointId destination_endpoint,
                                     ClusterId cluster_id, const uint16_t * attr_ids, uint16_t attr_id_count)
{
    BufBound buf = BufBound(buffer, buf_length);
    if (_encodeGlobalCommand(buf, mfgCode, destination_endpoint, cluster_id, 0x00))
    {
