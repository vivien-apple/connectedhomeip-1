...
namespace Inet {
    ...
    CHIP_ERROR UDPEndPointImplNetworkFramework::BindImpl(IPAddressType addressType, const IPAddress & address, uint16_t port,
        InterfaceId intfId)
    {
        ChipLogError(chipTool, "%s (%p)", __func__, this);
        ...
        const IPAddress & aAddress, uint16_t aPort)
    {
        ...
        /*
                ...
                if (strcmp(addrStr, "fe80::1%lo0") != 0 && strcmp(addrStr, "0.0.0.0") != 0 && strcmp(addrStr, "::") != 0 && strcmp(addrStr, "127.0.0.1") != 0 && strcmp(addrStr, "192.168.1.5") != 0) {
                    ChipLogError(Inet, "Original IP %s changed to ::1", addrStr);
                    target = localIPv6;
                } else if (strcmp(addrStr, "::") == 0) {
                    ChipLogError(Inet, "Original IP %s changed to ::1", addrStr);
                    // ChipLogError(Inet, "Replacing host %s for 0:0:0:0:0:0:0:0 with port: %s", addrStr, portStr);
                    target = localIPv6;
                }
        */
        const char * localIPv6 = "::1";
/*
        if (strcmp(addrStr, "fe80::1") != 0 && strcmp(addrStr, "0.0.0.0") != 0 && strcmp(addrStr, "::") != 0 && strcmp(addrStr, "127.0.0.1") != 0 && strcmp(addrStr, "192.168.1.5") != 0) {
            ChipLogError(Inet, "Original IP %s changed to ::1", addrStr);
            target = const_cast<char *>(localIPv6);
        }
*/
        (void)localIPv6;
        const char * testIP = "fe80::109a:df9:c731:b0ba%en11";
        if (aPort == 0 && strcmp(addrStr, "::") == 0) {
            target = const_cast<char *>(testIP);
        }
    ...
    CHIP_ERROR UDPEndPointImplNetworkFramework::GetConnection(const IPPacketInfo * aPktInfo)
    {
        ...
        __auto_type endpoint = GetEndPoint(mAddrType, aPktInfo->DestAddress, aPktInfo->DestPort);
