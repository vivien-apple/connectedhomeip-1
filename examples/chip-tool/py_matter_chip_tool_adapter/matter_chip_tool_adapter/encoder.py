...
class Encoder:
    ...
    def __get_command_name(self, request):
        ...
        if request.is_attribute:
            ...
            # chip-tool expose writable attribute under the "write" command, but for non-writable
            # attributes, those appears under the "force-write" command.
    ...
    def __maybe_add_destination(self, rv, request):
        if not self._has_destination_supports(request):
            return rv
    ...
    def __maybe_add_endpoint(self, rv, request):
        if not self._has_endpoint_supports(request):
            return rv
    ...
    def _has_endpoint_supports(self, request):
        return self._has_supports(request, 'has_endpoint')

    def _has_destination_supports(self, request):
        return self._has_supports(request, 'has_destination')

    def _has_supports(self, request, supports_name):
        ...
        if supports_name in aliases:
            return aliases.get(supports_name)
