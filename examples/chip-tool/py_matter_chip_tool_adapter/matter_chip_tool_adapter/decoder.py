...
class DarwinAnyFormatConverter(BaseConverter):
    ...
    def run(self, specs, value, cluster_name: str, typename: str, array: bool):
        if isinstance(value, list) and len(value) >= 1 and isinstance(value[0], dict) and value[0].get('data') is not None:
            value = [self.__convert_data_to_value(specs, item_value, cluster_name, typename) for item_value in value]
        return value

    def __convert_data_to_value(self, specs, value, cluster_name: str, typename: str):
            ...
            if isinstance(value, list):
              if value_type == 'Structure':
                  ...
                  for field in value:
                      ...
                      struct[str(context_tag)] = self.__convert_data_to_value(specs, field, cluster_name, typename)
                  value = struct
