                                          ...
                                          OperationalDeviceProxy * peer_device)
{
    ...
    case Clusters::Identify::Commands::TriggerEffect::Id:
        triggerEffectCommand.effectIdentifier = static_cast<Clusters::Identify::IdentifyEffectIdentifier>(data->args[0]);
        triggerEffectCommand.effectVariant    = static_cast<Clusters::Identify::IdentifyEffectVariant>(data->args[1]);
        Controller::InvokeCommandRequest(peer_device->GetExchangeManager(), peer_device->GetSecureSession().Value(), binding.remote,
